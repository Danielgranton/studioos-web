        ...
        >
            ...
            <div className="flex min-w-0 flex-1 flex-col p-2.5">
                ...
                <div className="mt-2" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
                    ...
                    <div className="mt-2 flex items-center gap-2"><span className="w-7 text-right font-mono text-[9px] text-[#777]">{formatTime(currentTime)}</span><div className="relative min-w-0 flex-1"><input aria-label="Preview position" type="range" min="0" max={trackDuration} step="0.01" value={Math.min(currentTime, trackDuration)} onChange={(event) => handleSeek(event.target.value)} style={{ background: `linear-gradient(to right, #fff 0%, #fff ${playedPercent}%, #777 ${playedPercent}%, #777 ${accessiblePercent}%, #ef4444 ${accessiblePercent}%, #ef4444 100%)` }} className="relative z-10 h-1 w-full cursor-pointer appearance-none rounded-full accent-white" />{accessiblePercent < 100 && <><span aria-hidden="true" className="pointer-events-none absolute top-1/2 z-20 h-2.5 w-px -translate-y-1/2 bg-red-400" style={{ left: `${accessiblePercent}%` }} /><span aria-hidden="true" className="pointer-events-none absolute -top-3 z-20 -translate-x-1/2 font-mono text-[8px] font-semibold text-red-300" style={{ left: `${accessiblePercent}%` }}>{formatTime(accessibleDuration)}</span></>}</div><span className="w-7 font-mono text-[9px] text-[#777]">{formatTime(trackDuration)}</span></div>
                </div>
