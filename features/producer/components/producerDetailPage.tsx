            ...
            <div className="mx-auto max-w-[1280px] px-6 py-7 lg:px-20 lg:py-10">
                ...
                <div className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,1fr)_300px]">
                    <div className="space-y-7">
                        <section className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between lg:gap-6"><div className="min-w-0 lg:flex-1"><Label>About the producer</Label><p className="mt-2 max-w-3xl text-xs leading-5 text-[#aaa69d]">{producer.bio || "A creative professional ready to help shape your next release."}</p></div><div className="flex flex-nowrap gap-2 overflow-x-auto lg:shrink-0"><ProducerMetric icon={<Users size={13} />} label="Followers" value={formatCount(producer.followerCount)} /><ProducerMetric icon={<Music2 size={13} />} label="Produced" value={formatCount(producer.releaseCount)} /><ProducerMetric icon={<Music2 size={13} />} label="Beats produced" value={formatCount(producer.beatCount)} /></div></section>
                ...
                <div className="mt-7"><ReviewList target="PRODUCER" targetId={producerId} /></div>
            </div>
        </main>
    );
}
...
function ProducerMetric({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) { return <div className="flex min-w-[102px] items-center gap-1.5 rounded-lg border border-[#2a2825] bg-[#161513] px-2.5 py-2"><span className="shrink-0 text-[#e8a33d]">{icon}</span><div className="min-w-0"><p className="text-[8px] uppercase tracking-[0.1em] text-[#6b685f]">{label}</p><p className="truncate text-xs font-semibold text-[#e5e1d8]">{value}</p></div></div>; }
