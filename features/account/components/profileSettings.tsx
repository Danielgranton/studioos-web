...
export function ProfileSettings() {
    ...
    async function saveProfile(event: React.FormEvent<HTMLFormElement>) {
        ...
        const payload: UpdateProfileRequest = {
            ...
            experience: readValue(form, "experience"), available: form.get("available") === "true", instagram: readValue(form, "instagram"), youtube: readValue(form, "youtube"), link: readValue(form, "link"),
        };
...
function ProfileForm({ profile, role, saving, onSubmit }: { profile: AccountProfile | null; role?: string; saving: boolean; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) { const canSetAvailability = role === "PRODUCER" || role === "ARTIST"; return <form onSubmit={onSubmit} className="mt-6 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Field label="Location" name="location" defaultValue={profile?.location} placeholder="Nairobi, Kenya" /><Field label="Genre" name="genre" defaultValue={profile?.genre} placeholder="Afrobeats, hip-hop..." /></div><Field label="Bio" name="bio" defaultValue={profile?.bio} placeholder="Tell the StudioOS community about yourself" multiline /><Field label="Experience" name="experience" defaultValue={profile?.experience} placeholder="Your creative experience" multiline />{canSetAvailability && <label className="flex items-center justify-between gap-4 rounded-xl border border-[#303030] bg-[#101010] px-4 py-3 text-sm"><span><span className="block font-medium text-[#ddd]">Available for work</span><span className="mt-1 block text-xs text-[#777]">Show as available on your public card.</span></span><input type="checkbox" name="available" value="true" defaultChecked={profile?.available !== false} className="h-4 w-4 accent-[#3ea6ff]" /></label>}<div className="grid gap-4 sm:grid-cols-3"><Field label="Instagram" name="instagram" defaultValue={profile?.instagram} placeholder="@username" /><Field label="YouTube" name="youtube" defaultValue={profile?.youtube} placeholder="Channel URL" /><Field label="Website" name="link" defaultValue={profile?.link} placeholder="https://..." /></div><div className="flex justify-end"><button type="submit" disabled={saving} className="rounded-xl bg-[#3ea6ff] px-5 py-2.5 text-sm font-semibold text-[#0f0f0f] transition hover:bg-[#65b8ff] disabled:opacity-60">{saving ? "Saving..." : "Save changes"}</button></div></form>; }
